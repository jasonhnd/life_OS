...
Life OS 的 AI 朝廷负责决策和执行，**第二大脑**负责持久化存储。每次对话可以是新开的，丞相会自动查阅历史档案，流程结束自动存档。

```
三省六部流程 → 奏折存决策库 / 行动项存任务库 / 报告存日志库
新对话 → 丞相搜索历史 → 带上下文继续
```

你可以用 Notion、Obsidian、Google Sheets 或其他平台搭建第二大脑。核心是 **领域数据库 + 任务/决策/日志/目标的关联**。六部是 AI 的分析框架，领域是你生活的实际分区，两者独立。

**[查看完整搭建指南 →](docs/second-brain.md)**（含 Notion 逐步建库教程、数据库字段表、其他平台适配方案）

不连接数据层时，Life OS 所有功能照常工作，只是没有跨会话记忆和自动存档。
...
├── docs/
│   └── second-brain.md         # 第二大脑搭建指南
├── references/
│   ├── departments.md          # 六部×四司详细职能
│   ├── scene-configs.md        # 10个标准场景配置
│   └── notion-schema.md        # Notion 数据库 schema（含 data source ID）
├── evals/                      # Eval 体系
│   ├── scenarios/              # 测试场景
│   └── rubrics/                # 评分标准
├── pro/
