...
hard_rules_violated:
  - SKILL.md L161 "Start Session / Review MUST read agents/retrospective.md and launch RETROSPECTIVE as subagent. HARD RULE."
  - SKILL.md L119-126 "ROUTER must NOT output any step's content itself."
  - hosts/CLAUDE.md L237 "Trigger words MUST load agent files... Never execute a role from memory without reading its definition file. HARD RULE."
  - agents/retrospective.md L50-65 "DIRECTORY TYPE CHECK — if SKILL.md + agents/ + themes/ exist → ask a/b/c"
...
**应该执行：**
1. ROUTER 读取 `agents/retrospective.md`
2. ROUTER 输出 2 行：
   ```
...
- **编造**了"3 行轻量简报路径"作为合法逃逸——SKILL.md / hosts/CLAUDE.md 全文都没有这条
...
- 按 retrospective.md L50-65 的 **DIRECTORY TYPE CHECK** 明文规则，dev repo 收到"上朝"应该：
  > 检测 SKILL.md + agents/ + themes/ → 识别为 LIFE OS SYSTEM REPOSITORY → 问用户：a) 连 second-brain  b) 开发模式  c) 新建 → 选 b 后 bind dev repo, **skip step 3-7**, jump to step 8
...
**重做 R6：**
- 双仓库 compliance/violations.md（dev repo `compliance/` + user `_meta/compliance/`）
- 完全 md + git，不依赖本地私有存储
...
**表现：**
- 收到 trigger 词后**不读** `agents/retrospective.md`
- **不 launch** RETROSPECTIVE 子代理
- 在主上下文自己执行 retrospective 18 步中的几步
- 跳过 step 2 directory type check（dev repo 处理路径）

**违反的 HARD RULE：**
- `SKILL.md:161` "Start Session / Review: MUST read `agents/retrospective.md` and launch RETROSPECTIVE as subagent. HARD RULE."
- `SKILL.md:119-126` "ROUTER must NOT output any step's content itself."
- `hosts/CLAUDE.md:237` "Trigger words MUST load agent files... Never execute a role from memory without reading its definition file. HARD RULE."
...
- 编造"3 行轻量简报路径"作为合法逃逸（SKILL.md / hosts/CLAUDE.md 全文均无此条）
...
| 层 | Bug | 现状 |
|---|-----|------|
| **文档** | HARD RULE 已经写在 SKILL.md + hosts/CLAUDE.md | ✅ 文档完整，无缺失 |
...
|---|------|----------|---------|---------|---------|
| **R1** | Dev repo hook | 新建 `.claude/settings.json` | UserPromptSubmit hook 检测 `^(上朝\|start\|begin\|閣議開始\|开会\|はじめる\|开始\|初める\|開始)` → 注入 `<system-reminder>HARD RULE: Launch(retrospective) as subagent in Mode 0. Do NOT execute any step in main context. Read agents/retrospective.md first.</system-reminder>` | A | 仅 dev repo（防开发者自己被糊弄） |
...
| **R3** | retrospective Mode 0 自检 | `agents/retrospective.md:23-25` 顶部 | 子代理启动第一句必须是：`✅ I am the RETROSPECTIVE subagent (not main context simulation). Reading agents/retrospective.md... Step 1: THEME RESOLUTION.` | A+B | 全平台分发 |
...
| **R5** | AUDITOR 扩展 | `agents/auditor.md` 加 "Start Session Compliance" patrol | retrospective Mode 0 完成后自动 launch auditor 做 compliance check：检查 ① ROUTER 是否只输出 2 行；② 子代理是否真被 launch；③ 18 step 是否子代理执行 | A+B | 全平台分发 |
...
# Hook 命令（伪代码）
# 当检测到违规时
COMPLIANCE_FILE=$(if [[ -f agents/retrospective.md ]]; then
  echo "compliance/violations.md"
else
...
```
compliance/violations.md           ← 最近 90 日
compliance/archive/
  2026-Q1.md
  2026-Q2.md
  ...
...
**新增文件：**
- `compliance/violations.md`（模板，含表头）
- `compliance/violations.example.md`（示例 + 格式说明）
- `references/compliance-spec.md`（规格文档：何时写入、何时升级、谁读取）
- `compliance/2026-04-19-court-start-violation.md`（本文件）

**修改文件：**
- `agents/retrospective.md` step 2 加：bind 后初始化对应路径的 `compliance/violations.md`
- `agents/auditor.md` 加 Compliance Patrol 模式
...
- [ ] R3: `agents/retrospective.md` Mode 0 加自检
- [ ] R5: `agents/auditor.md` 加 Compliance Patrol 模式
- [ ] R6: 建立 `compliance/violations.md` + `references/compliance-spec.md`
...
- [ ] 故意违规（在主上下文模拟 step）→ AUDITOR Compliance Patrol 应该捕获并 append 到 `compliance/violations.md`
...
- 看到 SKILL.md + agents/ + themes/ 这三件套 = 100% dev repo
...
- `~/life-os/hosts/CLAUDE.md`
- `~/life-os/agents/retrospective.md`
- `~/life-os/agents/auditor.md`
...
- `compliance/violations.example.md`
- `compliance/2026-04-19-court-start-violation.md`（本文件，已建）
...
| 文件位置 | `compliance/2026-04-19-court-start-violation.md` |
