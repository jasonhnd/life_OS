...
After GWT arbitrator returns `[COGNITIVE CONTEXT]`, orchestrator MAY also spawn `narrator` (v1.7 Phase 2) AFTER REVIEWER Final Review (between step 6 and step 7) to wrap Summary Report substantive claims with `signal_id` citations from the cognitive context. Narrator failure is non-blocking — falls back to v1.6.3 unwrapped Summary Report. See `pro/agents/narrator.md`.
