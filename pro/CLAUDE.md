...
When the user sends a non-Start-Session message AND `_meta/sessions/INDEX.md` exists in the bound second-brain, the orchestrator MAY launch the Pre-Router Cognitive Layer to produce annotated input for ROUTER. Activation is per-session (controlled by `_meta/config.md` `cortex_enabled: true|false`); default is **OFF / opt-in** for v1.7 (off until explicitly enabled). Set `cortex_enabled: true` in `_meta/config.md` to enable.
...
**Cost**: ~$0.05-0.10 per invocation (Opus token usage). Worth it on substantive decisions; wasteful on trivial chat. The OFF / opt-in default reflects this — users enable Cortex when they have ≥30 sessions of accumulated history.
