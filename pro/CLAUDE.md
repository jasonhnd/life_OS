...
**文件写入冲突规则**：六部并行执行时，每个部门只能修改自己负责的文件。如果多个部门需要修改同一个文件，必须串行执行——由尚书省在派发令中标注依赖关系，将涉及同一文件的部门放入不同的并行组。
...
谏官返回后，丞相检查输出中是否有"📝 模式更新建议"段落。如果有，将内容追加写入 `~/.claude/user-patterns.md`（附日期标签）。谏官自身保持 Read-only，不直接写文件。
...
流程结束后，丞相按 `notion-schema.md` 的存档操作章节执行：
