...
**跨层验证**：如果当前项目定义了 cognition 流转，检查本次 session 是否引用了上游 wiki 知识。如果没有 → 标注："⚠️ 已定义 cognition 流转但本次 session 未实际使用"
...
💭 自问：
- 两个不相关事物之间有什么会令用户惊讶的关联？
- 近期决策中完全缺失了什么维度？
- 如果 SOUL.md 存在，近期行为是否与声明的价值观一致？
- 用户的未来自我会希望今天注意到什么？

若 `_meta/STRATEGIC-MAP.md` 存在，还需检查：
- **结构性**：已定义的流转中，是否有任何已过时、失效或获得了新证据？
- **SOUL × 战略**：driving_force 是否与 SOUL 维度一致？是否有生活维度完全不在任何战略线中？
- **行为模式 × 战略**：行为模式（user-patterns.md）是否与战略优先级一致？用户是否在回避某个 critical-path 项目？
- **wiki × 流转**：cognition 流转是否实际承载了 wiki 知识？上游的条目是否被下游引用？
- **超越结构**：战略地图尚未捕捉到的关联是什么？

🎯 产出 1-3 条真正的洞见。质量优先。"未检测到显著的跨领域模式"是有效的——不要捏造。
...
- Phase 2 战略候选：[{列表} / 本次无]
- Phase 2 last_activity 已更新：[{涉及的项目}]
