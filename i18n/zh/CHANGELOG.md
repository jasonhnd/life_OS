...
- **翰林院升级为"人类智慧殿堂"** — 从三个思维工具扩展为 18 个领域、70+ 位思想家的深度对话系统
- **18 个领域**：科学与认知、哲学与思辨、东方思想、战略与领导、商业与决策、心性与修行、系统与自律、人性与洞察、文明与历史、逆境与反脆弱、美学与创造、政治与治国、经济与社会、数学与逻辑、医学与生命、探索与冒险、沟通与说服、法律与正义
- **深度角色扮演** — 每位思想家用自己的口吻说话，用自己的方法论思考，引用真实案例和著作
- **三种对话模式** — 单人深谈、圆桌会议、辩论。翰林院担任主持人
- **独立 subagent 架构** — 每位思想家作为完全独立的 subagent 运行，真正信息隔离
- **智能推荐** — 翰林院根据用户目的推荐思想家和对话方式
- **结尾沉淀** — 每位思想家给出角色内临别赠言 → 翰林院总结（思路变化、关键洞察、未解决问题）→ 写入 second-brain
- **开放名单** — 用户可点名任何不在列表上的历史人物

**门下省强化**
- **10/10/10 悔恨测试** — 三个时间尺度（10 分钟 / 10 个月 / 10 年）必须分别回答
- **红队审查** — 假设计划一定会失败，找出 3 个最可能的失败点
- **否决格式结构化** — 否决必须包含不通过维度、问题本质、修订方向、缺失信息

**政事堂正式化**
- **新建 agent 文件** — `pro/agents/zhengshitang.md`，3 轮结构化辩论规则
- **量化触发** — 两部评分差 ≥ 3 分或结论直接矛盾时自动触发
- **主持辩论** — 尚书省主持；陈述（≤3 句）→ 交锋（≤5 句）→ 最终陈述（≤2 句）→ 裁决

**尚书省智能调度**
- **依赖检测** — 自动检测部门间数据依赖，安排串行执行（Group A → Group B）
- **咨询机制** — 部门可通过尚书省请求其他部门的特定数据点

**丞相意图澄清**
- **问题分类策略** — 决策/规划/困惑/复盘/信息类，每类有针对性的澄清重点
- **情绪分离协议** — 先回应情绪，分离事实与感受后再判断是否升报

**谏官行为追踪深化**
- **行为模式学习循环** — 新模式发现、模式更新、模式消退，写入 user-patterns.md
- **跨会话趋势分析** — 对比 ≥ 3 份历史报告的风险偏好、决策速度、执行力、关注维度变化
- **正向强化协议** — 批评与表扬比例不低于 3:1，明确指出改善之处
