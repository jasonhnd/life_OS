...
- **触发词表** 写入 SKILL.md — 定义英文、中文、日文三种语言的触发词，覆盖 5 个动作（上朝/早朝/退朝/快速分析/朝堂议政）
- **"上朝"定义** — 全量同步拉取 + 朝前准备 + 巡检 + 早朝简报 + 等待指令。解决了 Claude 和 Codex 在用户说"上朝"时行为不一致的问题
- **"退朝"升级** — 现在包含向所有已配置后端全量同步推送，不仅限于 GitHub
- **早朝官 Mode 0（上朝）** 新增到 zaochao.md — 完整会话启动序列
- **早朝官 Mode 4（退朝）** 新增到 zaochao.md — 完整会话关闭 + 全量同步推送
- **早朝/复盘** 明确为仅出简报、不做全量同步（更快，适用于会话中期检查）
- **i18n 已同步** — 中文和日文 SKILL.md + zaochao.md 更新到 v1.4.2a
