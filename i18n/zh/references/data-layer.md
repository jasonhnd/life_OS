...
感知 → 捕获 → 关联 → 判断 → 沉淀 → 涌现
 ↑       ↑       ↑       ↑       ↑       ↑
手机    GTD   Zettelkasten 3D6M   PARA  Lint/御史台
体验   inbox/   wiki/    桌面   目录  健康巡查
```
...
**捕获 → 关联（Zettelkasten）**：桌面 CC 从收件箱拉取，第一步是构建关联——与哪些现有 wiki 文章相关？提到了哪些实体？实体之间有什么关系？用 wikilink 构建反向链接网络。

**关联 → 判断（三省六部）**：不是所有信息都需要决策。只有涉及重大资源分配、多选项权衡或难以逆转的后果时，才激活三省六部决策模式。

**判断 → 沉淀（PARA）**：决策结论按 PARA 存储——Projects（有终点的事项）、Areas（持续领域）、Wiki（跨领域知识）、Archive（已完成）。

**沉淀 → 涌现（Lint/御史台）**：知识网络密度达到临界质量时，自动发生涌现。Lint 扫描发现跨领域关联，建议新概念文章，找出矛盾。

### 移动 vs 桌面分工

移动端只负责感知和捕获（偶尔轻量关联，如网络搜索）。桌面负责关联、判断、沉淀和涌现——所有重活。移动端可以读取管道产出（STATUS.md、归档），但只在捕获阶段写入。
