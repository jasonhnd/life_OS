...
DREAM 是系统的离线记忆处理机制——灵感来自人类的睡眠周期。它作为**ARCHIVER（`pro/agents/archiver.md`）的 Phase 3** 在每次退朝时运行，扫描过去 3 天的活动，进行整理、巩固和发现。

> **注意**：DREAM 不再是独立 agent。v1.4.4 起已融入ARCHIVER。本规范定义三个阶段；ARCHIVER将其集成到退朝流程中。
...
用户说 "退朝" / "adjourn" / "終わり"
    ↓
RETROSPECTIVE：归档 + 同步 + 推送（模式 4）
    ↓
RETROSPECTIVE：启动 DREAM agent（最后一步）
    ↓
DREAM 运行三个阶段
    ↓
...
下次上朝，RETROSPECTIVE读取最新未呈现的梦境报告，并在晨报中包含：
