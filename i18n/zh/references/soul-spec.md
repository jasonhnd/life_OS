...
|------------|------|----------|
| < 0.3 | 新写入，数据点少 | 仅谏官引用 |
| 0.3 – 0.6 | 中等证据 | 谏官 + 门下省引用 |
| 0.6 – 0.8 | 充分证据 | + 中书省引用 |
| > 0.8 | 深度验证，低矛盾率 | 全系统引用（含丞相） |
...
| **谏官** | 工作流结束后，观察到反复出现的价值信号 | "你总是先问家庭影响" |
| **翰林院** | 深度对话中，用户通过交谈透露价值观 | 用户告诉苏格拉底"稳定比冒险更重要" |
...
| 角色 | 读取内容 | 使用方式 |
|------|---------|----------|
| **丞相** | 偏好、红线 | 更精准的意图澄清——即使用户未提及，也会就其关心的维度提问 |
| **中书省** | 价值优先级（confidence ≥ 0.6） | 如果用户未提及相关维度，自动将其添加为必要维度 |
| **门下省** | 实然与应然的差距（confidence ≥ 0.3） | 价值一致性检查——当决策与陈述的愿望矛盾时发出提示 |
| **谏官** | 所有条目，证据与矛盾数量 | 行为审计——强化或挑战 SOUL 条目，提议更新 |
| **翰林院** | 世界观、未解决的矛盾 | 推荐能够应对用户特定张力的思想家 |
...
- `user-patterns.md` 记录**你做了什么**——谏官观察到的行为模式
