...
- Strengths 与 Weaknesses 必须引用具体证据（引用 agent 输出、分数矛盾、跳过的 phase）。"所有 agent 表现都不错"之类的泛泛赞扬是 AUDITOR 反模式，`agents/auditor.md` 明令禁止。
...
- 每次完整审议工作流之后（`hosts/CLAUDE.md` Step 8）。
...
- `agents/auditor.md` —— eval-history 条目的唯一写入者（Step 8 末尾）
- `agents/retrospective.md` —— Mode 0 读者，做系统性模式检测（最近 10 个文件）
