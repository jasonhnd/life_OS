...
所有写入发生在单次 ARCHIVER 调用内。Orchestrator 不在阶段之间交错写入。这保留了在 `hosts/CLAUDE.md` 中定义的散朝状态机 —— ARCHIVER 在所有阶段完成时发射一份 Completion Checklist，会话结束。
...
每个 Cortex 组件都是 `agents/*.md` 中定义的 Claude Code 子 agent。执行规则与现有 16 个子 agent 相同。
...
| Agent | 用途 |
|-------|---------|
| `agents/hippocampus.md` | 跨会话检索，发射 memory signals |
| `agents/gwt-arbitrator.md` | 显著性竞争，发射带注释的输入 |
| `agents/narrator-validator.md` | Sonnet Claude Code 子 agent —— 引用校验器（Phase 2） |
...
- `agents/archiver.md` Phase 2 —— 新增概念提取、Hebbian 更新、method-candidate 检测、衰减过程、会话摘要写入、索引重建
- `agents/retrospective.md` Mode 0 —— 重新生成 `meta/concepts/INDEX.md` 与 `meta/sessions/INDEX.md`；标记休眠概念
- `agents/router.md` —— 在其输入中接受认知注释；在 Step 7.5 掌管 narrator 组合

Claude Code 在派生时读取每个 agent 定义；调用之间不持久化任何运行时状态。

### 信息隔离扩展（Information isolation additions）

`hosts/CLAUDE.md` 中现有的 Information Isolation 表在 v1.7 新增三行，保持同一原则 —— 每个 agent 只收到其工作所需的内容：
