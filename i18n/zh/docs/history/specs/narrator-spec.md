...
narrator 作为 **Step 7.5** 插入到 hosts/CLAUDE.md 工作流中，位于 Step 7 Summary Report 组装之后、Step 8 AUDITOR 之前：
...
ROUTER 自身处理 narrator 角色。**不存在独立的 `agents/narrator.md` 文件**。narrator 行为通过 hosts/CLAUDE.md 编排更新来配置，并存在于 ROUTER 的输出组装职责之内。
...
创建一个新子 agent 文件：`agents/narrator-validator.md`。
...
- `agents/router.md` —— narrator 组装位于 ROUTER 内部（本规范 §6）
