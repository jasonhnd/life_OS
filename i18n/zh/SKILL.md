...
**主题按会话独立** —— 每个对话窗口可以使用不同的主题，互不影响。主题选择不跨会话持久化。

首次开始会话时，RETROSPECTIVE agent 会呈现选择：
```
🎨 选择你的主题：
 a) 🏛️ 三省六部 —— 唐代治理体制（中文经典）
 b) 🏛️ 霞が関 —— 日本中央省庁（霞关）
...
可用主题：
- `themes/zh-classical.md` —— 三省六部（唐代治理体制）
- `themes/ja-kasumigaseki.md` —— 霞が関（日本中央省庁）
- `themes/en-csuite.md` —— C-Suite（企业高管架构）

用户可随时切换："切换主题" / "switch theme" / "テーマ切り替え"

**主题决定输出语言（硬规则）**：选择主题后，整个 session 的所有输出必须使用该主题的语言。zh-classical = 中文，ja-kasumigaseki = 日文，en-csuite = 英文。适用于每个 agent、每份报告、每次回复。用户切换主题时，输出语言立即切换。没有例外。
...
| 🎋 STRATEGIST | 人类智慧殿堂——93 思想家，18 个领域 | 需要时询问用户 |
