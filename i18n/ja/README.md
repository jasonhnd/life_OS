...
Life OS は AI ターミナル（Claude Code、Gemini CLI、Codex CLI）にインストールして使う**個人の内閣**だ。複数の独立した AI エージェントがあなたの意思決定をあらゆる角度から分析し、互いに議論し、計画*と*あなた自身の両方に責任を問う。
...
> **非対応**：ChatGPT、Gemini Web、その他のシングルコンテキストチャットインターフェース。Life OS は真の情報隔離を持つ複数の独立した subagent を必要とする——単一のチャットウィンドウでは実現できない。
...
各テーマは約60行のファイルひとつ。複数の機能 ID を表示名にマッピングし、語調を定義し、トリガーワードを設定し、出力フォーマットを命名する。エンジンはセッション開始時にテーマファイルを一度読み込み、以降はその名前をあらゆる箇所で使用する。
...
普通の AI と話すと、一つの声が返ってくる——自信に満ち、従順で、チェックを受けていない。Life OS は複数の声を与え、そしてそれらはいつも同意するわけではない。その緊張こそが核心だ。
