...
これはLife OSのClaude上での完全版です：16の独立したAI役職が真の情報隔離と並列実行を実現します。
...
4. 完了です！16のサブエージェントが自動的に準備され、全プロジェクトで利用可能になります
...
- 16のAI役職がそれぞれ独立に動作し、互いの思考過程を見ることができない
- 六部は並列実行可能（順番待ちなし）
...
Proモードが[Gemini CLI](https://github.com/google-gemini/gemini-cli)と[Google Antigravity](https://idx.google.com/)で利用可能になりました。Gemini 2.5 Proによる16の独立サブエージェントで動作します。
...
- 同じワークフロー、同じ情報隔離、同じ16の役職

---

## OpenAI Codex CLI（Proモード）

Proモードが[Codex CLI](https://github.com/openai/codex)で利用可能になりました。o3による16の独立サブエージェントで動作します。
...
> **注意**: Life OSはProモードが必要です — 16の独立サブエージェントと真の情報隔離。シングルコンテキストプラットフォーム（ChatGPT、Gemini Web、Claude.ai Web）はサポートされていません。
...
A：[Claude Code](https://claude.ai/code)を推奨します。16の独立サブエージェントによる完全なProモード、ワンコマンドインストールです。
