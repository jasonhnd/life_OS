...
>
> Wave 1 には R-1.8.0-020 の commit タイトルが修正済みと主張したのに実際は未修正だった macOS 裸 `python` 移植性修正、および v1.8.0 メンテナンスラインの R-1.8.0-021 / R-1.8.0-022 の 1.8.1 への正式取り込みも含む。
...
### 削除 · Zero-Python pivot（Wave 2 · 2026-05-02）

このクリーンアップを駆動したユーザーの原文：「我想把这些东西全砍掉。我不理解为什么需要向量数学，我的系统里也没有 FTS5。」翻訳：Layer 4 Python ツールはユーザーが要求も使用もしていない複雑度（FTS5、ベクトル数学、mypy/ruff/pytest CI gate、50+ オプショナル依存、LLM 駆動 prompt との二重メンテナンス負担）を背負っていた。Wave 2 はリポジトリの Python を一行残らず削除する。

**削除されたソースファイル（11 .py + 5 lib .py）**：
- `tools/approval.py`（1010 行 — セキュリティ guard；~40 個の危険パターンを bash に port、下記「変更 · セキュリティ guard が bash ネイティブに」参照）
- `tools/embed.py`、`tools/export.py`、`tools/reconcile.py`、`tools/research.py`、`tools/search.py`、`tools/seed.py`、`tools/sync_notion.py`、`tools/skill_manager.py`、`tools/stats.py`（以前 LLM prompt で置換済み；今削除）
- `tools/__init__.py` + `tools/lib/{__init__,config,llm,notion,second_brain}.py`
- `tools/README.md`（ドキュメント化すべき Python パッケージなし）

**削除されたテストスイート（12 テスト + 5 lib テスト）**：
...
**削除されたパッケージング**：
- `pyproject.toml`（公開すべき Python パッケージなし）
- `uv.lock`（ロックすべき依存なし）
- `.python-version`（ランタイム pin 不要）
- `.github/workflows/integration.yml`（integration テストの対象は削除済み Python research ツール）

**削除された eval シナリオ**（削除済み Python ツールが対象）：
- `evals/scenarios/tool-{embed,export,migrate,reconcile,research,search,seed,stats,sync_notion}.md`

**削除された「Python のラッパーだった bash」**：
- `scripts/wiki/wiki-link-audit.sh`（311 行の bash + awk frontmatter parsing）— `scripts/prompts/wiki-link-audit.md`（LLM 駆動 Glob + Grep + Read）に置換。

### 変更 · セキュリティ guard が bash ネイティブに（Wave 2 · 2026-05-02）

`scripts/hooks/pre-bash-approval.sh` は以前すべての Bash コマンドを `tools/approval.py` Python サブプロセスにブリッジしていた。Wave 2 はそのブリッジを完全に削除し、危険パターンコーパスを bash 正規表現配列としてインライン埋め込み（~40 エントリ；port 時に 5 つの Hermes 製品固有パターン削除 — gateway/cli.py/`hermes update` は上流ランタイム外では無関係）。100% 純 bash；jq で JSON parse（jq 欠如時の stdin 抽出フォールバックとして python3 を許容）。出処保持：NousResearch/hermes-agent (MIT) commit `59b56d4...` から fork → 以前は `tools/approval.py @ v1.7.3`。

`scripts/hooks/pre-write-scan.sh` は同じ理由で `scan_hermes_dangerous_patterns()` Python ブリッジを削除。直接実行する 15 個の prompt-injection / SQL / secret / PII 正規表現パターンは不変。

`tests/hooks/test_pre_bash_approval.sh` は新しい自己完結動作に合わせて書き直し：7 テスト / 9 アサーション、Windows MSYS bash、macOS bash 3.2、Linux bash 5.x で全合格。17 個のフィクスチャコマンド（10 基本 + 7 エッジケース、大文字 RM / sudo prefix / find -exec rm / .env redirect / YOLO bypass を含む）でスモークテスト済み。

### 変更 · CI workflow（Wave 2 · 2026-05-02）

`.github/workflows/test.yml` 書き直し：以前は pytest + mypy + ruff マトリックスで 3 OS × 2 Python バージョン = 6 ジョブ。今は単一の `bash-tests` マトリックス、3 OS（ubuntu / macOS / windows）。ステップ：`git ls-files '*.sh'` で各 `.sh` に `bash -n` 構文チェック；tests/hooks/ bash スイート；spec drift スキャン；pre-prompt-guard 正規表現スモーク；lifeos-compliance-check fixture。~3 倍速、外部 Python ツールチェーン不要。

### 変更 · Wiki frontmatter スキーマ（Wave 2 · 2026-05-02）

`scripts/wiki/setup-secondbrain.sh` テンプレート + `scripts/prompts/inbox-process.md` + `scripts/prompts/research.md` を v1.8.1 frontmatter に更新：

| フィールド | 旧 | 新 |
|---|---|---|
| `aliases` | (欠落) | `[]` 空配列；Obsidian が `[[wikilink]]` 解決に使用 |
| `source` | スカラー文字列 | `sources: []` 複数配列 |
| `confidence` | float `0.0–1.0` | enum `impossible \| unlikely \| possible \| likely \| certain` |
| `last_tended` | (欠落) | ISO 日付 — 最後に能動的に review した時点（cosmetic 編集ではなく） |
| `review_by` | (欠落) | ISO 日付 — `wiki-decay` がこのエントリを再表示すべき時点 |

マイグレーション prompt：`scripts/prompts/migrate-confidence.md`（vault 内で `/migrate-confidence` を実行）。冪等。マッピング：`< 0.3 → unlikely`、`0.3-0.6 → possible`、`0.6-0.85 → likely`、`>= 0.85 → certain`。

`/wiki-decay` 更新：`last_tended` + `review_by` + 5 段階 `confidence` を使ってエントリを `due-for-review` / `stale` / `borderline` / `active` に分類。オプショナルな `+ link audit` 分岐。レガシー float `confidence` はスキャン時に自動マッピング、レガシーエントリは強制マイグレーションなしで動作継続。

### 追加 · Wiki 出処タグ + 重複検出 + CitationAgent（Wave 2 · 2026-05-02）

- **`## Key facts` セクションの出処タグ**：各 bullet が `^[extracted]`（`sources[]` URL から言い換え）、`^[inferred]`（自分の合成、どのソースにも直接記載なし）、または `^[ambiguous]`（ソース間で不一致）を持つ。タグなしはデフォルトで `^[extracted]`。`/research` subagent が bullet ごとに出処を出力；`/inbox-process` は accept/update 時に必須要求；下流合成は保守的に伝播（extracted+extracted=extracted；競合=inferred または ambiguous に降格；上方向に昇格しない）。
- **`/inbox-process` LLM ベース重複検出 pre-pass**：SHA256 ハッシュ（バイト一致の重複は捕捉するが言い換え近似重複は見逃す）を置換。各 inbox 項目：トピックフレーズ抽出 → 候補ドメインのキーワードを `Grep` → 候補 frontmatter + TL;DR を `Read` → LLM がトピック重複と claim 重複を判断 → `accept` / `update` / `merge` / `accept with related-link` を提案。純 LLM、FTS5 なし、ベクトル数学なし。
- **`_meta/inbox/manifest.json` デルタ追跡**：どの inbox 項目が過去の実行で見られたかを追跡。各 `/inbox-process` 実行は提案テーブルの行に Δ-new vs carried-over をマーク、ユーザーが新規素材を優先できる。
- **切り離された CitationAgent（`/research` の Phase 4）**：Anthropic のマルチエージェント research-system アーキテクチャからのパターン。合成**後**に実行（インターリーブしない）。1 つの追加 `general-purpose` subagent が WebFetch で各 `^[extracted]` bullet が fetch 可能なソース URL に対応するか検証；verified / unverified / inferred-source-combos / ambiguous-disagreements を報告。未検証 `^[extracted]` bullet は自動的に `^[inferred]` に降格。`^[extracted]` claim の 30%+ が検証失敗した場合、`confidence` が一段階下がる。切り離しは分析の深さを保つ；インターリーブされた citation はモデルを浅く引用しやすい事実に押しやる傾向。`--no-citations` でオプトアウト可。コスト：counter-bias-only 比 +20-30% LLM 呼び出し。

### 追加 · 確証バイアス対抗ポジショニング（Wave 2 · 2026-05-02）

EN / ZH / JA の README に、Life OS Wiki が我々がベンチマークした 6 つの主要 LLM-Wiki / マルチエージェント research プロジェクト（Anthropic research-system ブログ、LangChain `langgraph` agent supervisor、GPT-Researcher、CrewAI、QX-Labs、OpenAI Deep Research）それぞれとどう異なるかを説明するセクションを追加。要約軸：(1) 統合 wiki backing store vs 一時的出力；(2) bullet ごとの出処タグ；(3) 切り離された CitationAgent；(4) 5 段階校正された confidence enum；(5) `last_tended`/`review_by` 鮮度モデル；(6) inbox トリアージ引き渡し vs 自治追加；(7) zero-python デプロイ姿勢。

### 検証（CI マトリックス）— Wave 2 更新

- 32 → 19 個の tracked .sh（`wiki-link-audit.sh` 削除 + Wave 2 クリーンアップ）`bash -n` → Linux + macOS + Windows MSYS で全合格
- `STRICT=1 bash scripts/check-spec-drift.sh` → exit 0
- `tests/hooks/*.sh`（8 スイート、~70 アサーション）→ 全合格
- `bash /c/tmp/run-hook-tests.sh`（17 危険 + 安全フィクスチャを新 bash approval guard で実行）→ 17/17 合格
- retag 後 `bash scripts/verify-release.sh` → 7/7 ✅
- mypy / ruff / pytest gate：**削除**（リポジトリに Python なし）

### マイグレーション — Wave 2 更新

```bash
cd ~/.claude/skills/life_OS && git pull
bash scripts/setup-hooks.sh   # 新 /inbox-process + /research + /migrate-confidence + /wiki-link-audit をインストール、hook 更新
```

その後 second-brain vault 内で任意の Claude Code セッションを開く。SessionStart hook が欠落している v1.8.1 スキャフォールディングを自動検出してサイレントに作成。レガシー float `confidence` を持つ既存 wiki エントリは引き続き動作 — `/wiki-decay` が自動マッピング。永続的にマイグレーションするには：vault 内で `/migrate-confidence` を実行（冪等、書き込み前に提案プレビュー）。

`python -m tools.<X>` 呼び出しを実行していたユーザーへ：それらの Python エントリポイントはもう存在しません。代替：
- `python -m tools.search` → `/search` slash コマンド（または自然言語「搜一下 X」）
- `python -m tools.embed` → no-op；concept インデックスは現在 `/extract-concepts` 経由のオンデマンド LLM 駆動
- `python -m tools.research` → `/research` slash コマンド（マルチエージェント、CitationAgent 付き）
- `python -m tools.export` → 代替なし；裸 markdown が export
- `python -m tools.reconcile` → 代替なし；`/wiki-decay` が鮮度の競合を表面化
- `python -m tools.sync_notion` → orchestrator が adjourn flow Step 10a で MCP 経由で Notion sync 処理
- `python -m tools.stats` → `/eval-history-monthly` slash コマンド
- `python -m tools.skill_manager` → `/method create|update|list` slash コマンド

`uv` / `pip install -e .` は不要。skill 自体には Python ランタイムインストール不要（jq が hook の優先 JSON parser；jq 欠如時の優雅なフォールバックとして python3）。
...
- **R-1.8.0-017 · スキャナを真に source of truth にする —— EXEMPT_PATTERN を絞り、本物の残留をキャッチ（2026-04-30 ユーザー第 11 ラウンド監査後）**: ユーザー第 11 ラウンド監査のコア洞察: R-1.8.0-016 のスキャナがパスしていたのは過広な**ディレクトリレベル免除**（`docs/architecture/`、`docs/guides/`、`i18n/.*/docs/`、`i18n/.*/references/` 全体がスキップ）のせいであって、リポジトリが本当にクリーンだからではなかった。あの免除下ではスキャナ PASS は "active ドキュメントにドリフトなし" の証拠にならない。監査者はスキャナが見落とした 24 件の active カウントドリフト残留（FAQ、テーマ概要、ZH/JA インストールドキュメントなど）を列挙して gap を証明。本ラウンドは (a) ディレクトリ免除を削除してスキャナを信頼可能にし、(b) 新スキャナがキャッチした全てを修正。
  ...
  - **Active ドキュメント修正（13 ファイル 28 substitutions）**: Python スクリプト + 個別ファイル編集で、新スキャナがキャッチした全ヒットをカウントフリー表現に置換。
    ...
    - `docs/reference/faq.md`: 4 箇所（`16 个独立 subagent`、`16 个 engine agent 的逻辑`、`16 个 AI 角色`）→ カウントフリー。
    - `docs/reference/version-history.md`: v1.0 エントリ `Pro 模式（14 个独立 subagent）` → `Pro 模式（独立 subagent）`（歴史的コンテキストは保持しつつ古いカウントを引用しない）。
    - `docs/user-guide/themes/{adding-a-theme,chinese-themes,english-themes,japanese-themes,themes-overview}.md`: 7 箇所の `16 个 engine ID …` → カウントフリー / `engine ID …`。
    - `i18n/zh/docs/installation.md`: 4 箇所の `16 个独立 subagent` → `多个独立 subagent`。
...
- **R-1.8.0-015 · Subagent 数ドリフトクリーンアップ + tag 再整列（2026-04-30 ユーザー第 9 ラウンド監査後）**: ユーザー第 9 ラウンド監査が R-1.8.0-014 STRICT 通過後にさらに 2 つの残存問題をキャッチ: (a) subagent 数ドリフト —— ルート `AGENTS.md` は既に 23 だが、`pro/AGENTS.md`、`pro/GEMINI.md`、`docs/index.md`、`docs/installation.md`、`SKILL.md`、3 言語 README および約 14 のユーザー向けドキュメントが依然として 16 と記載; (b) `v1.8.0` git tag が依然として `02aea0d`（R-1.8.0-013 commit）を指しており、最新 Spec-GC HEAD `6cb3d79` ではない。意味的なカウントドリフトは R-1.8.0-014 のスキャナがキャッチできない種類の問題 —— STRICT は削除済みパス / 禁止 token のみを検証し、ユーザー可視のセマンティック数字は検証しない。
  ...
  - **検証**: `STRICT=1 bash scripts/check-spec-drift.sh` exit 0; `git ls-files | xargs grep -l "16 subagents|16 个 subagent|16 个 agent|All 16 subagents"` フィルタリング後の非 legacy 非 CHANGELOG ファイル: **active 0 ヒット**; mypy --strict tools/ → 0 エラー / 16 ファイル; ruff → クリーン; pytest → 232 合格 / 3 deselected; 31 個の tracked .sh `bash -n` すべて合格。
