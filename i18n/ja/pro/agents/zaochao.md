...
10.5. 戦略マップコンパイル:
   ...
   b. _meta/strategic-lines.md を読む → 全ライン定義を取得
   c. 各ラインについて:
      - projects/*/index.md から一致する strategic.line を持つプロジェクトをスキャン
      - 各プロジェクトの project_health を計算（ステータス + 活動 + タスク）
      - line_health を計算（役割による加重平均）
      - ボトルネックを検出（critical-path の停滞または enabler のブロック）
      - 劣化を検出（役割別の活動閾値 — strategic-map-spec.md 参照）
   d. 全 flows_to/flows_from からフローグラフを構築
   e. 意思決定伝播の警告をチェック（前回コンパイル以降の意思決定）
   f. _meta/STRATEGIC-MAP.md を書き込む（コンパイル済み）
   g. ブリーフィングに含める: 戦略概要 + 警告（出力形式を参照）
10.6. Wiki 健全性チェック:
...
[絵文字] [ライン名] ([ヘルス]/10 [指標]): [一行のステータス]
  [ツリー] [プロジェクト] — [指標] [ステータス] ([役割], [活動])
  [ツリー] [プロジェクト] — [指標] [ステータス]
[警告] グローバルボトルネック: [プロジェクト] — [なぜ重要か]
(strategic-lines.md がない場合 → このセクションを省略)
