...
8.5. 戦略マップコンパイル:
   a. _meta/strategic-lines.md が存在しない場合 → 省略（戦略的関係未定義）
   b. _meta/strategic-lines.md を読む → 全ライン定義を取得（driving_force、health_signals を含む）
   c. 全 projects/*/index.md を読む → strategic フィールドを収集
   d. 各ラインについて:
      - strategic.line が一致するプロジェクトを収集し、role でソート（critical-path 優先）
      - ヘルスアーキタイプをマッチ（strategic-map-spec.md 参照）: 🟢 steady / 🟡 controlled wait / 🟡 momentum decay / 🔴 uncontrolled stall / 🔴 direction drift / ⚪ dormant
      - ナラティブを記述: 何が起きているか + 何を意味するか + アクションへの示唆
      - ブラインドスポットを検出: 途切れたフロー、劣化、情報不足
   e. クロスレイヤー検証:
      - SOUL × 戦略ライン: driving_force が SOUL ディメンションと整合しているか？
      - wiki × フロー: cognition フローのドメインに wiki コンテンツがあるか？下流が参照しているか？
      - user-patterns × 役割: 行動が戦略的優先事項と整合しているか？
   f. アクション推奨を生成（🥇 最高レバレッジ / 🥈 注目に値する / 🟢 無視して安全 / ❓ 決定が必要）
   g. _meta/STRATEGIC-MAP.md をコンパイル
...
🗺️ 戦略概要 (strategic-lines.md が存在する場合):
[絵文字] [ライン名]                    [ヘルスアーキタイプ指標]
   ウィンドウ: [期限] ([N 週]) | ドライビングフォース: [driving_force]
   [プロジェクト]   [役割]   [ステータス指標]
   ナラティブ: [何が起きているか + 何を意味するか]
   → アクション: [今日への示唆]
(strategic-lines.md がない場合 → フラットなエリアステータスリストにフォールバック)

⚡ 今日:
🥇 [最高レバレッジ]: [理由] | 工数: [時間] | 不作為のコスト: [何が起きるか]
🥈 [注目に値する]: [理由]
🟢 無視して安全: [リスト]
❓ 決定が必要: [リスト]
...
6.7. _meta/STRATEGIC-MAP.md を読む（存在する場合） → 戦略コンテキストとして丞相に渡す
...
6. ~/second-brain/_meta/STRATEGIC-MAP.md で戦略ラインのヘルス傾向を読む（存在する場合）
```
...
🗺️ 戦略ヘルス (STRATEGIC-MAP.md が存在する場合):
[絵文字] [ライン名] ([ヘルスアーキタイプ]): [前回レビューとの傾向]
  🚧 ボトルネック: [プロジェクト] — [理由] (該当する場合)
  🔴 劣化: [プロジェクト] — [N日間無活動] (該当する場合)
