...
すべてのトリガーは **24 時間の重複抑制**の対象です — 同じトリガーが過去 24 時間以内にすでに発火していれば（`meta/journal/*-dream.md` を参照）、スキップされます。

#### 1. new-project-relationship

- **データソース**: `projects/*/index.md` 戦略フィールド、`meta/strategic-lines.md`
...
- **データソース**: `meta/decisions/*/*.md`（v1.9 月次サブディレクトリ）+ プロジェクト決定履歴
- **ハード閾値**: 同じ質問/主題が実行のないまま 3 回以上決定されている
- **ソフトシグナル**: 反復と認識されないよう、ユーザーが質問を言い換える
- **自動アクション**: 次回ブリーフィングのフラグ（「また X を決定しています — コミットメントを避けていませんか？」）
- **重複抑制**: 過去 24 時間以内に発火した場合は抑制

すべてのフラグは `meta/journal/{date}-dream.md` に構造化された `triggered_actions` YAML ブロックで書き込まれます。RETROSPECTIVE は次の Start Session 時にこれを読み、関連するフラグをブリーフィングに表示します。
...
`meta/journal/{date}-dream.md` に書き込まれます：
