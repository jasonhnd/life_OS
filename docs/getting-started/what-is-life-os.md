...
├── wiki/                   # 你知道什么（可复用知识）
├── projects/{name}/        # 你在做什么（活跃 + archived 项目，靠 lifecycle_stage frontmatter 区分）
├── areas/                  # 你持续关注什么（领域；README.md 列推荐种子，不强制命名）
├── meta/
...
> v1.9 变化：无独立 `archive/` 目录（归档用 frontmatter `lifecycle_stage: archived`，项目留在 `projects/`，保护 wikilinks）；`meta/user-patterns.md` 移入 `meta/`；`meta/` 不带下划线。详见 `docs/second-brain.md`。
...
- **Layer 4（LLM 驱动 prompt 层 · v1.8.1+）**：原 Python 工具层在 v1.8.1 Wave 2（zero-python pivot, 2026-05-02）整体删除。维护任务现在全部由用户触发的 slash command + ROUTER 读 markdown prompt 直接执行，工具是 Read/Write/Glob/Grep（不是 Python 子进程）：
  ...
  - git 同步：ARCHIVER 在 Phase 4 直接 `git add + commit + push`（`hosts/CLAUDE.md`）
