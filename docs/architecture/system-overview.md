...
Cortex **不是新一层**。它是在 v1.6.2a 稳定核心之上叠加的 Layer 2 增量：在现有 multiple agents 之前增加 **Pre-Router 认知前置层**（Step 0.5），并扩展 ARCHIVER Phase 2：
...
[GWT 仲裁]（salience 竞争 + top-K 广播）
  ↓
Annotated input → ROUTER（原 multiple agents 流程不变）
  ↓
…Summary Report…
  ↓
...
|------|-----------|--------|
| **第二大脑**（知识持久化） | Layer 1 | — |
| **决策引擎**（multiple agents 工作流） | Layer 2 | — |
...
- **Layer 2**：multiple agents + 9 theme + GLOBAL.md 三平台共用；唯一差异在编排文件（CLAUDE.md / GEMINI.md / AGENTS.md）
