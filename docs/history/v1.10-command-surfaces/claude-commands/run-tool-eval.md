...
Per Stage 9, `evals/regression-fixtures/*.md` (28 cases; markdown with YAML frontmatter — converted from `.yml` in v1.8.5 Stage 9 / v1.8.6 per the md-only constraint) are negative cases — they MUST FAIL when run through validators. A separate `/run-regression` flow handles those (planned — see `regression-from-violation.md`; until a dedicated command ships, validate negative fixtures by running them through the validators directly). `/run-tool-eval` covers POSITIVE tool scenarios only.
