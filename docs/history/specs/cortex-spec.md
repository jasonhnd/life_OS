...
>
> For v1.8.0 canonical guidance, read `hosts/CLAUDE.md` §0.5. This file is preserved as historical context for understanding the v1.7 → v1.8 evolution; do not treat any "always-on", "MUST spawn", or "narrator-validator" claim below as current operational policy.
...
All writes happen inside a single ARCHIVER invocation. The orchestrator does not interleave writes between phases. This preserves the Adjourn state machine defined in `hosts/CLAUDE.md` — ARCHIVER emits one Completion Checklist when all phases are done, and the session ends.
...
Every Cortex component is a Claude Code subagent defined in `agents/*.md`. The execution rules are the same as the existing 16 subagents.
...
| Agent | Purpose |
|-------|---------|
| `agents/hippocampus.md` | Cross-session retrieval, emits memory signals |
| `agents/gwt-arbitrator.md` | Salience competition, emits annotated input |
| `agents/narrator-validator.md` | Sonnet Claude Code subagent — citation validator (Phase 2) |
...
- `agents/archiver.md` Phase 2 — add concept extraction, Hebbian update, method-candidate detection, decay pass, session summary write, index rebuild
- `agents/retrospective.md` Mode 0 — regenerate `meta/concepts/INDEX.md` and `meta/sessions/INDEX.md`; flag dormant concepts
- `agents/router.md` — accept cognitive annotation in its input; own narrator composition at Step 7.5
...
The existing Information Isolation table in `hosts/CLAUDE.md` gets three new rows in v1.7, preserving the same principle — each agent receives only what its job requires:
...
- Workflow orchestration → `hosts/CLAUDE.md`
