...
status: legacy
authoritative: false
superseded_by: hosts/CLAUDE.md + agents/auditor.md
note: "v1.7-era / pre-R-1.8.0-011 pivot. Read for historical context only; current behavior in hosts/CLAUDE.md. ALL scripts/*.sh detection commands below were retired in v1.8.5 Stage 2 — AUDITOR now runs these checks as inline LLM procedures (see agents/auditor.md)."
---
...
|---------|------|-----------|
| Life OS dev repo (`agents/retrospective.md` exists) | `compliance/violations.md` | Hooks + AUDITOR in dev repo |
...
After Step 11 (SOUL trend), **read** `compliance/violations.md` (dev repo) or `meta/compliance/violations.md` (user repo):
...
- Entries older than 90 days → moved to `compliance/archive/YYYY-QN.md` (quarterly rolling file)
- Archival triggered by `tools/backup.py` or manual:
  ```bash
  # Manual quarterly rotation (bash)
  q=$(( ($(date +%-m)-1)/3 + 1 ))
  ARCHIVE="compliance/archive/$(date +%Y)-Q${q}.md"
  # ... filter rows older than 90 days from violations.md to $ARCHIVE ...
  ```
- `git log compliance/violations.md` retains full history independently of this rotation
...
- Path: `compliance/YYYY-MM-DD-{incident-slug}.md`
- Linked from violations.md Details field: `(see compliance/2026-04-19-court-start-violation.md)`
...
- `compliance/violations.md` (dev repo) — **public** via GitHub. No user data.
...
- `compliance/violations.md` — live log (dev repo)
- `compliance/violations.example.md` — entry format examples
- `compliance/2026-04-19-court-start-violation.md` — COURT-START-001 incident archive
- `agents/auditor.md` — Compliance Patrol Mode specification
