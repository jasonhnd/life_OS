...
status: legacy
authoritative: false
superseded_by: hosts/CLAUDE.md + _meta/rfc/v1.9-second-brain-structure-optimization.md
note: "v1.7-era / pre-R-1.8.0-011 pivot. Read for historical context only; current behavior in hosts/CLAUDE.md and v1.9 RFC."
---
...
Method candidates are detected by `archiver` Phase 2 (see `agents/archiver.md`) using the same infrastructure that produces wiki and SOUL candidates.
...
- `agents/archiver.md` — writes method candidates (Phase 2)
- `agents/dispatcher.md` — injects methods into domain briefs
