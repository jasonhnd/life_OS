...
- Strengths and Weaknesses must reference concrete evidence (quoted agent output, score contradictions, skipped phases). Generic praise like "all agents performed well" is an AUDITOR anti-pattern and is banned by `agents/auditor.md`.
...
- After every full deliberation workflow (Step 8 in `hosts/CLAUDE.md`).
...
- `agents/auditor.md` — sole writer of eval-history entries (end of Step 8)
- `agents/retrospective.md` — Mode 0 reader for systemic pattern detection (last 10 files)
