...
2. **决策引擎** — 多个独立 agent 分权制衡,重大决策有人敢拍桌子
...
**单一 git 仓库**：本地工作副本(磁盘上的文件,同时也是你的 Obsidian vault)+ GitHub 远端(备份 + 跨设备同步)。不是三个后端,就是 git：
- 本地工作副本：当前活跃文件系统,本地直接读写,Obsidian 打开
- GitHub 远端：所有历史版本可追溯、可回滚,会话开始 `git pull`、会话结束 `git push`
- 跨设备：在另一台机器 `git clone` / `git pull`；冲突就是普通 git merge 冲突
...
- **git 跨设备同步** — 手机上经 git 投递到 `inbox/` 的条目,会话开始 `git pull` 自动拉回本地,下一次桌面 session 处理；会话结束 `git push` 把当次产出推回远端。无独立同步引擎。
...
- 多个 agent 制衡(Hermes 是单 agent + delegate,没有结构化 veto)
...
- Cortex **不是** Layer 5。Cortex 是 Layer 2 的内部升级——在 ROUTER 之前加一套认知前置流水线(海马体 + GWT + 叙事层)。从外面看,还是一个 Skill 层。Skill 层的输入从"用户原始 message"变成"带认知标注的 message",multiple agents 的编排流程本身不变。
...
- **决策引擎**活在 Layer 2(多个 agent 编排)+ Layer 1(读 SOUL / Wiki / Strategic Map)。
...
早上 9:00,Mac 前。场景：昨晚你在手机上经 git 往 `inbox/` 记了一条「研究 Japan permanent residency 要求」。
...
RETROSPECTIVE 经 `git pull` 拉回昨晚的 inbox 条目、读 SOUL / user-patterns / Strategic Map、版本检查、巡查、编晨报。
...
ARCHIVER 返回 Completion Checklist。Orchestrator 在主 context 执行 `git add + commit + push`(把 outbox 合并产出推回 GitHub 远端；离线则提交保留本地,下次 session 同步)。
...
- **总时长约 46 分钟**,其中你的键盘操作不超过 5 分钟,剩余 41 分钟系统在并行审议、在写文件、在 git 同步、在跑 hook、在做 DREAM 反思——这些都是不打扰你的。
...
**vs Obsidian**：它做知识管理比 Life OS 强。Life OS 的 wiki 是引擎自动提取的,不是你手敲的。**互补而不是替代**——用 Obsidian 看 Life OS 写的 markdown,体验最好。事实上 Life OS 的本地工作副本(git 仓库)就是用 Obsidian vault 的布局,所有 `[[wiki-link]]` 语法能直接跳转。
...
| 认知深度 | 弱(单 agent + MEMORY.md 扁平记忆) | 强(多个 agent 制衡 + 三层长期记忆 + Cortex 跨 session 记忆 / 概念图谱 / 方法库 / eval-history 自反馈) |
...
Hermes 执行力强认知弱；Life OS 认知强,执行力在 v1.7 GA 已补齐。方向是**吸收 Hermes 的执行模式到 Layer 3 + Layer 4**,保留 Life OS 的 multiple agents 制衡和三层记忆。两者不是零和竞争,是互补。

---

## 不支持什么(What Life OS doesn't do)

**单一上下文聊天界面**：ChatGPT、Gemini Web、Claude.ai 的网页版都不行。Life OS 需要多个真正独立的 subagent 带信息隔离,一个聊天框做不到。硬塞所有"独立审查"都会变成同一个上下文里的角色扮演,REVIEWER 看得见 PLANNER 的思考过程,veto 就变成走形式。必须在支持真 subagent 的平台上跑：Claude Code / Gemini CLI / OpenAI Codex CLI。
...
每次看到 multiple agents、9 themes、4 层架构、数十条 HARD RULEs（按 host 计数,详见 `references/hard-rules-index.md`）、Cortex 前置、v1.7 的 Hermes 级执行——都会想：是不是过度工程了？
...
- **multiple agents** 是原理 1 的分权具体化(PLANNER 不能 execute、REVIEWER 只能 veto 不能 rewrite、六部不能互审)
...
- **数十条 HARD RULEs（按 host 计数,详见 `references/hard-rules-index.md`）** 是这些原理在具体操作上的硬边界(intent clarification 不能跳、SOUL 要引用不能忽略、git sync 不能静默失败、Adjourn 不能半途退出、Privacy Filter 不能"人工把关"……)
