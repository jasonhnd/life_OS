...
> 实现现场：`agents/archiver.md` §Phase 2 Mid-Step（Concept Extraction + Hebbian Update）与 `tools/migrate.py` 使用相同的 6 条标准枚举。本规范改动时，两者必须同步更新。
...
这些阈值原本隐式在 `archiver` Phase 2 和 `agents/knowledge-extractor.md` 代码路径中。v1.8.7 **在本 spec 中显式化**，让它们可审计、可调、对未来读者可见无需读代码。**v1.8.6 行为不变** —— 仅文档表层提升。
...
- `agents/archiver.md` —— Phase 2 拥有 Hebbian 更新、衰减通道、SYNAPSES-INDEX 重新生成、tentative 写入
- `agents/retrospective.md` —— Mode 0 重新生成 `INDEX.md` 并标记 dormant 概念
