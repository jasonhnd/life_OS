...
status: legacy
authoritative: false
superseded_by: agents/narrator.md
scope: references
...
narrator は hosts/CLAUDE.md ワークフローに **Step 7.5** として挿入され、Step 7 Summary Report 構成の後、Step 8 AUDITOR の前に来ます:
...
ROUTER 自身が narrator の役割を担います。**別個の `agents/narrator.md` ファイルはありません**。narrator の動作は hosts/CLAUDE.md の orchestration 更新によって設定され、ROUTER の出力構成責務の内部に存在します。
...
新しいサブエージェントファイルが作成されます: `agents/narrator-validator.md`。
...
- `agents/router.md` — narrator の構成は ROUTER の内部に存在する(本 spec §6)
