...
- Strengths と Weaknesses は具体的な根拠(引用されたエージェント出力、スコアの矛盾、スキップされたフェーズ)を参照しなければなりません。「全エージェントが good performance を示した」のような一般的な褒め言葉は AUDITOR の anti-pattern であり、`agents/auditor.md` により禁止されています。
...
- full deliberation workflow(`hosts/CLAUDE.md` の Step 8)終了後すべて。
...
- `agents/auditor.md` — eval-history エントリの唯一の writer(Step 8 終盤)
- `agents/retrospective.md` — システミックパターン検出のための Mode 0 reader(直近 10 ファイル)
