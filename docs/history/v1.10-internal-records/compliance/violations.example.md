...
```
| 2026-04-21T10:30+09:00 | start | A2 | P1 | In dev repo (SKILL.md + agents/ + themes/ detected), retrospective Mode 0 skipped Step 2 DIRECTORY TYPE CHECK. User was not asked a/b/c. | false |
```

**Detection rule**:
- When `cwd` contains all three: `SKILL.md` + `agents/` + `themes/`
...
```
| 2026-04-21T11:15+09:00 | 上朝 | B | P0 | Claimed "轻量简报路径 exists in CLAUDE.md" but full-text search of CLAUDE.md / hosts/CLAUDE.md / SKILL.md shows no such path defined. | false |
```
...
```
| 2026-04-21T16:00+09:00 | 退朝 | E | P0 | ROUTER executed archiver Phase 2 logic in main context: output contained "扫描 wiki 候选" + listed 3 candidates with evidence counts. This is archiver subagent's job per hosts/CLAUDE.md Step 10. | false |
```
...
awk -F'|' '$5 ~ /P0/' compliance/violations.md | head -20

# All unresolved (false / partial) violations
grep -E '\| (false|partial) \|' compliance/violations.md

# Count by type
awk -F'|' 'NR>5 {gsub(/ /,"",$3); count[$3]++} END {for (t in count) print t, count[t]}' compliance/violations.md | sort

# 30-day rolling (requires date math)
cutoff=$(date -v-30d +%Y-%m-%d 2>/dev/null || date -d "30 days ago" +%Y-%m-%d)
awk -F'|' -v c="$cutoff" 'NR>5 && $2 > c' compliance/violations.md
```

## See Also

- `compliance/violations.md` — live log
...
- `agents/auditor.md` — Compliance Patrol Mode (writes to violations.md)
