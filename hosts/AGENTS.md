...
> **v1.8.0 pivot completed (R-1.8.0-013 round-6 audit)**: Cortex §0.5 rewritten to pull-based; Session Modes updated to user-invoked (cron removed); narrator-validator references replaced with inline self-check; skeleton script blocks removed. The authoritative spec remains `hosts/CLAUDE.md` — if a conflict surfaces, treat `hosts/CLAUDE.md` as source of truth.
...
When reading agent files from `agents/*.md`, apply these mappings:

| Agent Frontmatter Field | Original (Claude) | Codex Equivalent |
|------------------------|-------------------|------------------|
| `model: opus` (judgment tier) | Claude's strongest model | Codex's strongest available model (auto-select) |
| `model: sonnet` (execution tier) | Claude's mid tier | Codex's mid tier |
| `model: haiku` (batch tier) | Claude's cheapest/fastest tier | Codex's cheapest/fastest tier |
...
On Codex CLI, subagents are spawned as independent parallel agents. Each agent file in `agents/` is a self-contained subagent definition.
...
Security: See hosts/GLOBAL.md — 4 inviolable security boundaries
...
> **v1.8.0 pivot (R-1.8.0-011)**: Cortex always-on was REMOVED. Previous v1.7.2 design spawned 3 cognitive subagents on every user message (~$0.05-0.10 per turn even when LLM didn't need historical context). v1.8.0 replaces this with **pull-based** activation: ROUTER decides per-message whether to launch any Cortex subagent based on the actual question. Canonical guidance in `hosts/CLAUDE.md` §0.5.

**Subagents available (ROUTER spawns on demand)** — never automatic:

1. `hippocampus` — cross-session memory retrieval via 3-wave spreading activation. Useful when user asks "what did we decide about X" / "remember the discussion about Y". See `agents/hippocampus.md`.
2. `concept-lookup` — direct concept-graph match. Useful when user mentions a known concept that has accumulated context. See `agents/concept-lookup.md`.
3. `soul-check` — relevant SOUL dimensions. Useful when message touches values / identity / recurring patterns. See `agents/soul-check.md`.
4. `gwt-arbitrator` — only spawned if ≥2 of the above produced signals. See `agents/gwt-arbitrator.md`.

**ROUTER Step 7.5 (narrator mode)**: ROUTER-internal composition that wraps Summary Report substantive claims with `signal_id` citations IF and ONLY IF cognitive context exists for the current turn. Composition template at `agents/narrator.md` (ROUTER-internal template, NOT spawnable via Task).

The `narrator-validator` subagent that previously audited citation discipline was REMOVED in R-1.8.0-011 (file `agents/narrator-validator.md` deleted). Narrator now self-checks via inline rules in `agents/narrator.md` — citation failures fall back to unwrapped Summary Report, no validator-rewrite cycles.
...
ROUTER does not interject between phases. The subagent emits the Completion Checklist when done. See `agents/archiver.md` for the full specification.
...
**Health Check / First-Run Help** ("检查 Life OS" / "检查一下 LifeOS" / "我刚装好 Life OS" / "下一步怎么开始" / "上朝之前先自检" / "帮我修一下配置" / "check Life OS" / "is Life OS working" / "I just installed Life OS" / "health check" / "diagnose Life OS" / "始め方を案内"): ROUTER reads `scripts/prompts/doctor.md` and runs the Doctor workflow inline. This is natural-language first-run / health-check handling, not business deliberation. Do not redirect the user to `/doctor`; Doctor is read-only by default and ends with the next sentence the user can say.
...
The "session binding" rule constrains **data persistence**, NOT **discussion scope**. ROUTER engages directly with whatever the user raises.

ROUTER MUST NOT deflect with phrases like "本窗口角色只做 X" / "请转到其他窗口" / "translate to planner trigger paste for another window" / "召唤翰林院 panel" without user explicitly asking for it. When user raises a business question, ROUTER engages directly via existing triage rules.

If the user EXPLICITLY asks for external escalation, ROUTER may help craft that handoff. Cross-project decisions when made should be labeled "Cross-project decision" and written to all affected projects.
...
All agents must follow `hosts/GLOBAL.md` — security boundaries, upstream output protection, research process display, progress reporting, and universal anti-patterns.
...
6. **Trigger words MUST load agent files** — when a trigger word activates a role (Start Session → retrospective, Adjourn → archiver), the orchestrator MUST read the corresponding `agents/*.md` file and spawn it as a real subagent. Never execute a role from memory without reading its definition file. HARD RULE.
7. **AUDITOR Compliance Patrol auto-trigger** (v1.6.3b) — after every `retrospective` Mode 0 (Start Session) completes OR every `archiver` returns, the orchestrator MUST spawn `auditor` in Mode 3 (Compliance Patrol). Mode 3 audits the just-completed flow against the 7-class violation taxonomy (A1/A2/A3/B/C/D/E) and writes detected violations to `compliance/violations.md` (dev repo) or `meta/compliance/violations.md` (user repo). All-pass output: `🔱 [theme: auditor] · ✅ Compliance Patrol passed`. **Non-blocking (v1.9.2+)**: Mode 3 runs *after* the briefing renders and MUST NOT delay it — the user is unblocked the moment the briefing shows; findings surface async via `meta/review-queue.md` / the next briefing. Cannot be skipped. HARD RULE.
...
> See `hosts/CLAUDE.md` §"Decision Records" for full spec. Codex sync — same v1.9 schema applies.
...
> See `hosts/CLAUDE.md` §"Minimality Rule" for full spec. Codex sync — same 6-question gate applies.
...
> See `hosts/CLAUDE.md` §"Risk Domains" and `references/risk-domains.md` for full spec. Codex sync — same 8 domains apply.
