...
failure_modes:
  ...
  repair_actions:
    ...
    - "Append violation row to compliance/violations.md"
---
...
Reading agents/retrospective.md. Starting Step 0.5: primary-source scan.
...
2. Emit: `⚠️ VIOLATION: retrospective Mode 0 must be launched as an independent subagent via Task tool, not executed inline. Re-launch required. See compliance/2026-04-19-court-start-violation.md.`
...
4. AUDITOR Compliance Patrol (Mode 3) — post-hoc audit, writes to `compliance/violations.md`
...
## Step 0 · Pre-Session Runtime Readiness (HARD RULE · v1.8.5+)

The bash hook layer was retired in v1.8.5 Stage 2, and v1.8.7 DR-10 made md-only the ontological constraint. Step 0 is now a lightweight readiness marker, not a hook installer or hook-health probe.

Required Step 0 behavior:
- Emit `🚀 starting · retrospective · <mode>` as the E9 status line.
...
Briefing requirement: The briefing's H2 "## 0. ${RETRO_NAME} · 上朝准备 (runtime readiness + version markers + Cortex status, 3-5 lines max 10)" MUST include the Step 0 runtime readiness line above.
...
2. DIRECTORY TYPE CHECK [v1.8.0 R-1.8.0-011 · retrospective executes inline]
   - If current directory contains SKILL.md + agents/ + themes/:
...
v1.8.0 execution: RETROSPECTIVE runs both Step 8a + 8b commands directly via Bash tool. Previous v1.7.1 R10 pre-fetch via `retrospective-mode-0.sh` was removed in R-1.8.0-011 (script deleted in Option A pivot). Output goes into briefing's `## 0. ${RETRO_NAME} · 上朝准备 (runtime readiness + version markers + Cortex status, 3-5 lines max 10)` block.
...
Briefing `## 0. ${RETRO_NAME} · 上朝准备 (runtime readiness + version markers + Cortex status, 3-5 lines max 10)` MUST contain BOTH literal markers:
...
16. DREAM REPORT [LLM judgment] — read latest meta/journal/*-dream.md (if exists, not yet presented):
    ...
    - R11 AUDIT TRAIL: before proceeding to Step 17, write `meta/runtime/<sid>/retrospective-step-16.md` via inline md write with YAML frontmatter (v1.8.6 R13 md audit trail; pre-v1.8.5 used `scripts/lib/audit-trail.sh emit_trail_entry` — .sh retired; per DR-10 audit trails are .md, not .json). Audit trail frontmatter MUST additionally include `dream_triggers_validated: [{trigger_id: N, trigger_type: HARD|SOFT, task_path: ..., task_status: ..., task_lookup: found_at_path|renamed_candidate|not_found, promoted_to_focus: true|false, reason: "..."}]` — one entry per HARD trigger; empty array `dream_triggers_validated: []` when zero HARD triggers.
...
|----|------|------------------------|
| lifeos-001 | Maintenance overdue check | 10 maintenance jobs timestamps (per `hosts/CLAUDE.md` §"Maintenance jobs") |
...
| lifeos-007 | Gotchas review (v1.8.7 new) | `gotchas.md` cross-referenced with files/code touched in last 7 days |
...
- Report Step 0 runtime readiness: hook layer retired, inline enforcement active, no hook setup action required.
