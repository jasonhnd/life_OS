...
context_manifest:
  source_of_truth: [SOUL.md, references/soul-spec.md, most recent meta/journal/<sid>-soul-snapshot.md]
  supporting: [current_user_message, current_project]
  forbidden: [agents/hippocampus.md, agents/concept-lookup.md, snapshots beyond most recent (RETROSPECTIVE's job)]
blast_radius:
  allowed_scope: [meta/runtime/<sid>/soul-check-*.md]
...
- Modify SOUL.md, snapshots, or any user/domain file (read-only — all SOUL mutations happen in ADVISOR / archiver Phase 2). The only permitted write is the R11 audit trail at `meta/runtime/<sid>/soul-check.md`.
...
- Read more than the latest snapshot from `meta/snapshots/soul/` (older snapshots are RETROSPECTIVE's job for trend computation).
...
Glob `meta/snapshots/soul/*.md`, take the most recent by filename (snapshot_id format is `YYYY-MM-DD-HHMM`, sorts naturally). Parse to compare against current SOUL.md (detect tier transitions).
...
All failures log to `meta/eval-history/soul-check-{date}.md`.

---

## Anti-patterns (AUDITOR flags these)

- Modifying SOUL.md or snapshot files (read-only contract; `meta/runtime/<sid>/soul-check.md` audit trail is the only exception)
