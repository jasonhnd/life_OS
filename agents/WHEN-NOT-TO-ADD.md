...
4. **Per-session state, audit trails, gotchas** — e.g. "this session's archiver output". → Goes to: `meta/runtime/<sid>/` (audit trails) or `gotchas.md` (lessons learned).
...
Per `hosts/CLAUDE.md` Minimality Rule (v1.8.5 Stage 7), ask the 6 questions first:

1. Could a **rule** (in `hosts/CLAUDE.md`) accomplish this?
