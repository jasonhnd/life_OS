...
context_manifest:
  source_of_truth: [hosts/CLAUDE.md, hosts/GLOBAL.md, SOUL.md, references/risk-domains.md, references/data-layer.md]
  supporting: [references/scene-configs.md, scripts/prompts/doctor.md, themes/*.md, meta/queue/notifications.md, meta/STATUS.md]
  forbidden: [agents/planner.md, agents/reviewer.md, agents/archiver.md, agents/retrospective.md]
blast_radius:
  allowed_scope: [meta/runtime/<sid>/router-*.md, ~/.claude/lifeos-memory/<key>.md]
  forbidden_scope: [SOUL.md, wiki/, agents/, .claude/settings.json, decisions/]
failure_modes:
...
**Health check / first-run guidance**: When the user asks if Life OS is installed,
ready, broken, how to begin, why saving did not happen, or asks to check/fix the
current setup, read `scripts/prompts/doctor.md` and run the Doctor workflow
inline. This is a natural-language capability, not a slash-command requirement.
Do not tell the user to type `/doctor`. Doctor is read-only by default, reports
