...
context_manifest:
  source_of_truth: [hosts/CLAUDE.md, hosts/GLOBAL.md, references/compliance-spec.md, references/failure-taxonomy.md, references/audit-trail-spec.md, references/soul-spec.md, references/wiki-spec.md, references/agent-spec.md]
  supporting: [compliance/violations.md, meta/runtime/]
  forbidden: [agents/reviewer.md (AUDITOR audits REVIEWER, not vice-versa)]
blast_radius:
  allowed_scope: [meta/runtime/<sid>/auditor-*.md, compliance/violations.md (append-only)]
  forbidden_scope: [SOUL.md, wiki/, agents/, decisions/, .claude/settings.json]
failure_modes:
...
Automatic post-hoc audit to detect HARD RULE violations introduced by COURT-START-001 (2026-04-19). Runs after `retrospective` Mode 0 or `archiver` completes. Writes violations to `compliance/violations.md` (dev repo) or `meta/compliance/violations.md` (user repo). Specification: `references/compliance-spec.md`.
...
5. Cross-check each `output_summary` against the ROUTER paste markers and required report headings in the transcript. For retrospective, compare against `[STEP N · ...]` markers and the final briefing sections. For archiver, compare against Phase 1-4 report headings. For Cortex, compare against the transparent YAML payload and GWT `[COGNITIVE CONTEXT]` block.
...
3. **A2 check** — In dev repo (has `agents/retrospective.md`), does retrospective output contain "a) 连接到 second-brain" or equivalent a/b/c menu? Missing → log A2.
...
5. **B check (escape route fabrication)** — grep transcript for phrases like "轻量简报路径", "lightweight briefing path", "Pre-Court Preparation section". If any is referenced as existing in SKILL.md / hosts/CLAUDE.md / .claude/CLAUDE.md → verify via grep; if not found → log B.
...
- This is additive to `compliance/violations.md` / `meta/compliance/violations.md`; do not replace violation logging with eval-history.
...
Write findings to `compliance/violations.md` (dev repo) or `meta/compliance/violations.md` (user repo) per Mode 3 conventions. Each violation row carries both Mode-3 class (A/B/C/D/E/F if applicable) and F1-F17 tag per `references/failure-taxonomy.md`.
...
Audits all `agents/*.md` files against `references/agent-spec.md` v2 standard. Triggered:
...
Missing any → emit `F3 SCHEMA_FAILURE: agents/<file> missing v2 field: <field>`
...
- Tool used but NOT in list → emit `F12 DRIFT_FAILURE: agents/<file> uses <tool> but it's not in tools list`
- Tool in list but NEVER used in last 30 days → emit warning `agents/<file> declares <tool> but unused in 30 days; consider removal`
...
- Any match → emit `F10 RESPONSIBILITY_FAILURE: agents/<file> wrote to forbidden path <path>; v2 blast_radius violation`
...
For each agent referenced in `compliance/violations.md` history:
- Collect all violation types where this agent was implicated
- Compare against frontmatter `failure_modes.known`
- Any historical violation not represented in `known` → emit warning `agents/<file> failure_modes.known is missing entry for historical violation type <X>; update spec`
...
#### M7-1: `gotchas.md` exists with seed content

- File `gotchas.md` exists
...
- Single writer rule honored: no human commits to `gotchas.md` outside memory-keeper runs (check git log author / commit message pattern)

Missing or short → `F3 SCHEMA_FAILURE: gotchas.md missing or under-seeded (≥10 required)`
...
- `agents/archiver.md` contains a `## Phase 5 — Memory Keeper` section
- `agents/archiver.md` `Adjourn Report Completeness Contract` section says "seven core H2 headings" (not "six")
- `agents/archiver.md` Completion Checklist includes a `Phase 5 gotchas` line
...
| M8-4 | Every `agents/*.md` contains `## Status Output (E9)` section declaring all 8 statuses (with N/A explicit for non-applicable) | `F3 SCHEMA_FAILURE: incomplete Status Output declaration in <agent>.md` |
