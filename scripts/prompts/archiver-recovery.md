# Cron prompt · archiver-recovery (v1.8.0)

You are an autonomous cron-triggered Claude Code session. Your job is to detect missed adjourns and auto-recover them. No user is watching this run; output is captured to log + written to `_meta/inbox/notifications.md`.

## Context

This prompt fires daily at 23:30 local time via launchd / cron. The user may have:
- (a) Properly said "退朝" / "adjourn" today → you should detect this and exit silent (no recovery needed).
- (b) Forgotten to say adjourn → no archiver Phase 1-4 ran → recovery needed.
- (c) Said adjourn but archiver failed (CLASS_C violation in `pro/compliance/violations.md`) → recovery needed.
