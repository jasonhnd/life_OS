...
### 12. (v1.10.0 NEW · issue #4 D2) Degradation-safety tier gate

Per `references/model-dispatch-policy.md` + `.claude/commands/run-eval.md` §3. A scenario failing at its declared `min_model_tier` is a spec bug and blocks release verification.

```bash
grep -rl 'min_model_tier:' evals/scenarios/*.md | wc -l    # annotated scenario count
```

- Read `docs/evals/tier-matrix.md` (generated by `/run-eval --tier`):
  - Any row with verdict ❌ (scenario FAILED at its declared floor) → **FAIL** — release MUST NOT proceed; either raise the scenario's `min_model_tier` claim or simplify the prompt, then re-run `/run-eval --tier <floor>`.
  - All rows ✅ or `pending` → **PASS**.
  - Matrix has no recorded runs yet → **WARN** (v1.10.0 grace level — surface `⚠️ no tier runs recorded; run /run-eval --tier execution to populate docs/evals/tier-matrix.md`; escalates to BLOCK once the first tier run lands).

## Output format

After running all 12 checks, emit a summary:
...
12. Degradation-safety tier gate (WARN when no tier runs yet): ✅/⚠️/❌
