...
def trigger_recovery(data_root: Path, job: str) -> tuple[bool, str]:
    # v1.8.0 fix (R-1.8.0-007): run-cron-now.sh lives in the skill source,
    # NOT in the user's data root. The previous implementation looked for it
    # under data_root/scripts/run-cron-now.sh and would either fail with
    # "not found" or — worse — pick up a stale copy left in the data root by
    # an earlier install (which still had the bash-4 declare -A bug from
    # R-1.8.0-002). Resolve the script via __file__ so we always invoke the
    # current upstream version, then pass the data root via LIFEOS_DATA_ROOT
    # so the script knows which second-brain to operate on.
    skill_root = Path(__file__).resolve().parent.parent
    script = skill_root / "scripts" / "run-cron-now.sh"
    if not script.is_file():
        return False, f"run-cron-now.sh not found at {script}"
    try:
        env = os.environ.copy()
        env["LIFEOS_DATA_ROOT"] = str(data_root)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not (root / "_meta").is_dir():
        print(
            f"Error: {root} does not look like a Life OS data root (no _meta/)",
            file=sys.stderr,
        )
        return 2
